...
async def on_message(message):
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            if not db_channel and not isinstance(message.channel, DMChannel):
                logger.error(f"message channel name is type{type(message.channel)}")
                db_channel = Channel(
            ...
            if db_channel and not isinstance(message.channel, DMChannel):
                db_message = Message(
